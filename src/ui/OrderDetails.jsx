                ...
                <td className="px-3 py-4" colSpan={5}>
                    <ul className="flex flex-col">
                        {orderDetails.length > 0 && (
                            orderDetails.map((orderDetail, index) => (
                                <ul key={index} className="flex justify-between mb-1">
                                    ...
                                    <li className="italic text-[15px]">{`${orderDetail?.type ? orderDetail?.type + ', ' : ''} ${orderDetail?.size ? 'size: ' + orderDetail?.size + ', ' : ''} ${orderDetail?.color ? orderDetail?.color : ''}`}</li>
                                    {/* <li>{`${prod?.price} đ`}</li> */}
                                </ul>
                            ))
                        )}
                        ...
                        <li className="flex mb-1">
                            ...
                            <span className="text-red-500 font-[500]">{Number(totalPrice) + Number(deliveryFee || 0)}</span>
                        </li>
                        <li className="flex mb-1">
